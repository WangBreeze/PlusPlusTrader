...
def test_python_custom_indicators():
    ...
    try:
        ...
        if ema_indicator:
            ...
            # 测试指标更新
            test_prices = [100.0, 102.0, 101.0, 105.0, 103.0, 108.0]
            
            for price in test_prices:
                value, signal = ema_indicator.update(price)
                print(f"    价格: {price:.2f}, 指标值: {value:.2f}, 信号: {signal}")
...
def test_end_to_end_workflow():
    ...
    try:
        ...
        elapsed = monitor.stop_timer("data_processing")
